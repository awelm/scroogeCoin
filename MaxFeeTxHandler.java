...
public class MaxFeeTxHandler {
    ...
    public double calculateFee(Transaction t) {
        ...
        for(Transaction.Input i: t.getInputs()) {
            UTXO unspentTx = new UTXO(i.prevTxHash, i.outputIndex);
            Transaction.Output unspentOutput = utxoPool.getTxOutput(unspentTx);
            inputAmount += unspentOutput.value;
        }
     ...
     * updating the current UTXO pool as appropriate. The set of transactions returned should
     * produce the maximum possible mining fee
     */
    public Transaction[] handleTxs(Transaction[] possibleTxs) {
        ...
        // sort transactions by descending fee, and then continue as before
        final HashMap<Transaction, Double> txCost = new HashMap<Transaction, Double>();
        for(Transaction t: possibleTxs)
            txCost.put(t, calculateFee(t)); 
        Arrays.sort(possibleTxs, new Comparator<Transaction>() {
            public int compare(Transaction t1, Transaction t2) {
                return Double.compare(txCost.get(t2), txCost.get(t1));
            }
        });
